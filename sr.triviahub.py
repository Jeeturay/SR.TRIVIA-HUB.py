...
client = Bot(description="HQ Trivia Bot!", command_prefix="/", pm_help = True)
...
async def on_ready():
    ...
    print('--------')
    print('--------')
    print('Started Steevo')
    print('Created by Steevo!')
...
async def on_member_join(member):
    ...
    channel = discord.utils.get(client.get_all_channels(), server__name='HQ Trivia Bot!', name='bot-servers-join-leave-log')
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
...
async def iamdark(ctx):
    author = ctx.message.author
    await client.delete_message(ctx.message)
    role = discord.utils.get(ctx.message.server.roles, name='Steevo!')
...
async def iamshreyas(ctx):
    ...
    role = discord.utils.get(ctx.message.server.roles, name='Steevo!')
    await client.add_roles(ctx.message.author, role)
    print('Added Steevo! role in ' + (ctx.message.author.name))
    await client.send_message(author, embed=embed)
...
async def ownerinfo(ctx):
    embed = discord.Embed(title="Information about owner", description="Bot Name- HQ Trivia Bot!", color=0x00ff00)
    embed.set_footer(text="©Made by Steevo#3838 | HQ Trivia Bot.")

    embed.set_author(name=" Bot OwnerName- Steevo#3838")
    embed.add_field(name="Site- https://discord.gg/cMq96ER", value="Thanks for joining our server.", inline=True)
    await client.say(embed=embed)
...
async def help(ctx):
    ...
    embed.add_field(name = '``Our Help Server Link`` ',value ='https://discord.gg/cMq96ER',inline = False)
    ...
    embed.add_field(name = 'HQ Trivia Bot',value ='Thanks For Joining This Server HQ Trivia Bot..!!.',inline = False)
    await client.send_message(author,embed=embed)
...
async def modhelp(ctx):
    ...
    embed.add_field(name = 'd!membercount(Kick members Permission Required) ',value ='Use it like ``/membercount`` to get membercount',inline = False)
...
@client.command(pass_context=True)
async def buy(ctx):
    embed = discord.Embed(title="**Hey There!**", description="Check Out The Following Details!", color=0x00ff00)
    embed.set_footer(text="©Made by Steevo#3838 | HQ Trivia Bot.")
...
async def buy(ctx):
    ...
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = discord.Embed(color = discord.Color((r << 16) + (g << 8) + b))
    embed.add_field(name = 'First Plan ',value ='40rs 10 Days Access!.',inline = False)
    embed.add_field(name = 'Second Plan',value ='100rs 1 Month Access!',inline = False)
    embed.add_field(name = 'You Can Purchase Any Plan So Go Prime Plans!',value ='Make Tickets And Purchase And Give Payment Screenshot.',inline = False)
    await client.send_message(author,embed=embed)
    await client.say('📨 Check DMs For Information')
